...
flowchart TD
    ...
    Mempool -->|Speed-Up: Same Nonce<br/>+ Higher Gas| Replaced([Replaced: Emits Replacement Hash])
...
In naive component code, developers simply await the receipt of the initial hash:

```ts
// Naive component handler:
const receipt = await publicClient.waitForTransactionReceipt({ hash: H1 });
```

Although modern lower-level libraries like `viem` provide internal replacement detection (via block scanning and the `onReplaced` callback), **a naive component relying on local `useState` breaks down in production**:

1. **State Desynchronization:** The component's local `hash` state remains bound to the superseded $H_1$. If the UI renders an explorer link (`etherscan.io/tx/{hash}`), it directs users to an evicted transaction that returns a 404 error.
2. **Component Isolation:** Even if an inline `onReplaced` callback intercepts $H_2$, an isolated button component cannot propagate the replacement hash to global navigation bars, activity drawers, or notification feeds.
3. **Premature Error Rejection:** If the wallet user submits a cancellation or the RPC poll times out before the replacement is mined, the promise rejects with an unhandled exception—displaying a failure modal even though the replacement transaction $T_2$ was mined successfully.

Pulsar solves this by intercepting Viem's `onReplaced` event at the adapter level, updating the transaction record in the central persistent store (`status = TransactionStatus.Replaced`, `replacedTxHash = H2`, `pending: false`), and emitting the replacement hash via the `onReplaced` callback for application-level handling.
  ...
  const handleSwap = async () => {
    await executeTxAction({
      ...
      onReplaced: (newTx, oldTx) => {
        // Pulsar flags oldTx as Replaced (pending: false), persists replacedTxHash, and fires callback
        console.warn(`[Pulsar] Transaction ${oldTx.txKey} was replaced by: ${newTx.replacedTxHash}`);
      },
      onError: (err, tx) => {
...
| **User Speeds Up / Cancels** | ❌ Naive promise rejects or desyncs from replaced hash. | ✅ Intercepts Viem `onReplaced`; marks `Replaced`, persists `replacedTxHash`, and emits callback. |
