...
2. **Nonce-Aware Replacement Detection:** The tracker must listen for replacement events. When `H1` is replaced by `H2`, the store updates the record to `TransactionStatus.Replaced`, registers `replacedTxHash`, and transparently redirects tracking to `H2`.
...
const initialStore = createPulsarStore<TransactionUnion>({
  ...
  adapter: [
    pulsarEvmAdapter(wagmiConfig, appChains),
...
Instead of calling `walletClient.sendTransaction` inside a component closure, dispatch the operation to the Pulsar state machine:
...
export function SwapActionButton({ amount }: { amount: string }) {
  const executeTxAction = usePulsarStore((state) => state.executeTxAction);

  const handleSwap = async () => {
    await executeTxAction({
      actionFunction: async () => {
        // Execute the wallet interaction; returns the initial transaction hash (Hex)
        const hash = await walletClient.sendTransaction({
          to: ROUTER_ADDRESS,
      ...
      params: {
        ...
        description: [
          ...
          'Gas fee was increased; tracking replacement hash...',
        ],
        payload: {
          tokenIn: 'USDC',
          tokenOut: 'ETH',
      ...
      onReplaced: (newTx, oldTx) => {
        console.warn(`[Pulsar] Replaced ${oldTx.txKey} with ${newTx.txKey}`);
      },
      onError: (err, tx) => {
