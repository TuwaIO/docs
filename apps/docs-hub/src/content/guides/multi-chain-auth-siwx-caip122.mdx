...
| **Smart Contract Accounts** | Supported via EIP-1271 (`isValidSignature`) (deployed contracts)            | Not supported (Keypairs only; PDAs are off-curve & keyless)       |
| **Verification Engine**     | Viem `recoverAddress` (EIP-191) + RPC `readContract` (EIP-1271)             | Native Web Crypto API `crypto.subtle.verify` (Ed25519, Zero Deps) |
...
> [!NOTE]
> In full-stack TUWA applications, developers can import either from granular packages (`@tuwaio/nova-connect`, `@tuwaio/siwx-react`, `@tuwaio/satellite-*`) or from unified metapackage subpaths (`@tuwaio/sdk`, `@tuwaio/evm-sdk`, `@tuwaio/solana-sdk`). Both provide identical API surfaces.
...
import type { SiwxSession, SiwxSessionRecord, SiwxSessionStore } from '@tuwaio/siwx-server';
import { redisClient } from '@/lib/redis';

// Atomic Redis session store conforming to SiwxSessionStore
const sessionStore: SiwxSessionStore = {
  ...
  async create({ session, ttlSeconds }: { session: SiwxSession; ttlSeconds: number }): Promise<SiwxSessionRecord> {
    ...
    await redisClient.set(`siwx:sess:${id}`, JSON.stringify(record), 'EX', ttlSeconds);
    return record;
  },
